...
const h = location.href,
      boturl='__cpo',
      sketcherurl='territorial.io' //'51.158.204.66';

class TerraSeizer {
    static botPage = h.includes(boturl)&&!h.includes(sketcherurl);
    static playerName = this.botPage ? "test" : "GameSketcher";
    static run() {
        ...
        h.includes(boturl)&&!h.includes(sketcherurl)&&(()=>{
            ...
            GM_onMessage('Support',(s,rand) => {
                console.log('asdasdasdsads',[144, unsafeWindow.TerraSeizer.gamePlayers.indexOf(`${TerraSeizer.playerName}${TerraSeizer.botNumber}`), unsafeWindow.TerraSeizer.gamePlayers.indexOf(s)])
                unsafeWindow.TerraSeizer.send([144, unsafeWindow.TerraSeizer.gamePlayers.indexOf(`${TerraSeizer.playerName}${TerraSeizer.botNumber}`), unsafeWindow.TerraSeizer.gamePlayers.indexOf(s)]);
            });
        })();
    ...
    static injectHTML=async()=>{
        try{
            ...
            document.write((await(await fetch(location.href)).text())
                           ...
                           //.replace(/"Player "\s*/g, `"${this.playerName}"`)
                           ...
                           <button id="getSupport" style="position: fixed; top: 15px; right: 15px; z-index: 2147483647; padding: 10px 18px; border: none; border-radius: 8px; background: linear-gradient(135deg, rgb(255, 65, 108), rgb(255, 75, 43)); color: rgb(255, 255, 255); font-size: 14px; font-weight: bold; cursor: pointer; box-shadow: rgba(0, 0, 0, 0.3) 0px 4px 10px; transition: 0.3s;">🚀 GET SUPPORT</button>`)
                           .replace('aNL.removeChild(a3S);',`a3S.id!=='getSupport'&&aNL.removeChild(a3S);`)
                          );
            document.close();
